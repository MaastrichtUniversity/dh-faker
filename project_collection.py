...
def create_subjects(configuration, fake):
    ...
    for counter in range(configuration["number_of_subjects"]):
        subject = {
            "Subject": {"@id": "http://www.orpha.net/ORDO/Orphanet_fake_" + str(counter), "rdfs:label": fake.word()},
            "subjectSchemeIRI": {"@value": fake.word()},
            "subjectIRI": {"@value": fake.word()},
...
def create_contributors(configuration, fake):
    ...
    for counter in range(configuration["number_of_contributors"]):
        ...
        contributor = {
            ...
            "contributorName1": {"@value": f"{first_name} {last_name}"},
            "givenName": {"@value": first_name},
            "familyName": {"@value": last_name},
            "contributorContact": {"@value": fake.ascii_company_email()},
